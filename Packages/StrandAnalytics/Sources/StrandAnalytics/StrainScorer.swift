...
public enum StrainScorer {
    ...
    static func edwardsTRIMP(_ hr: [HRSample], restingHR: Double, hrReserve: Double,
        ...
        var acc = 0.0
        for i in hr.indices {
            let zone = zoneWeight(Double(hr[i].bpm), restingHR: restingHR, hrReserve: hrReserve)
            if zone > 0 {
                acc += Double(zone) * durations[i]
            } else if pctHRR(Double(hr[i].bpm), restingHR: restingHR, hrReserve: hrReserve)
                        >= ambientThresholdPctHRR {
                // Ambient credit rides the SAME per-sample gap as a real zone hit (#950), so a sparse
                // 30 s stream and a 1 Hz stream accrue the same baseline for the same wall-clock minute.
                acc += ambientWeight * durations[i]
            }
        }
        return acc
    }
