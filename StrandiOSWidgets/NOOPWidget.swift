...
struct NOOPWidgetView: View {
    ...
    ///
    /// Upstream's `systemMedium` layout. Kept verbatim so it keeps merging cleanly, but THIS fork routes
    /// `.systemMedium` to `overviewMedium` below (the WHOOP-style Sleep/Recovery/Strain card).
    ...
    /// The Recovery ZONE tint (green / amber / red at the 67 / 34 cut-offs), which is what every other
    /// Recovery surface shows. Only `overviewMedium` reads it — the upstream layouts use the flat domain
    /// accent `chargeColor` above, and both stay as they were.
    private var recoveryColor: Color {
        guard let r = snap.recovery else { return StrandPalette.textTertiary }
        return r >= 67 ? StrandPalette.statusPositive
            : r >= 34 ? StrandPalette.statusWarning
